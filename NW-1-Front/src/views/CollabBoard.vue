...
const getAllBoards = async () => {
    try {
        ...
        boardStore.setBoards({
            personal: data.personal_BOARD || [],
            collab: data.collaborate_BOARD || []
        });
        // if (personalBoards.length === 1 && collabBoards.length === 0) {
        //     router.push({ name: "TaskBoard", params: { id: personalBoards[0].id } });
        // } else {
        //     boardStore.setBoards({ personal: personalBoards, collab: collabBoards });
        // }
    } catch (error) {
                                ...
                                <tbody class="font-semibold">
                                    <tr v-for="(board, index) in boards.collab" :key="board.id"
                                        ...
                                        <td @click="router.push({ name: 'TaskBoard', params: { id: board.id } })"
                                            ...
                                            <td class="itbkk-access-right text-center px-6 py-4"> {{ board.accessRight }}</td>
